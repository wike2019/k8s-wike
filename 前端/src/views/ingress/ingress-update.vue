     ...
     <el-form :inline="true"  :model="form" ref="formRef"  >
       <el-tabs v-model="mode" @tab-click="Update">
         <el-tab-pane label="可视化展示" name="json">
           <mateData @input="getData($event,'metadata',form)" ref="mateDataRef" type="ingress"   :nameDisable="true" :nameRequired="true"></mateData>
...
export default defineComponent({
  ...
  setup(){
    ...
    async function getDataItem(){
      try {
        await changeNs(route.query.namespace)
        let tData=await ingressItem(route.query.namespace,route.query.name)
        ...
        state.name=state.form.metadata.name
        state.namespace=state.form.metadata.namespace
        state.apiVersion=state.form.apiVersion
        state.Kind=state.form.Kind
      }catch (e){
        console.log(e)
      }
    }
    ...
    return {...toRefs(state),checkData,inArrayWithMsg,postNew,getsvcnameList,formRef,mateDataRef,yamlRef,fixdata,removePaths,removeTls,addPathCfg,rmPathCfg,yamlChange,getData,InitDefaultBackend,Update,requireRules,addTls,removeHost,addHost,addPaths}
  }
})
</script>
