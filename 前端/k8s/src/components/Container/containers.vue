      ...
      <el-form :inline="true"  :model="form" ref="formRef"  >
        <div v-for="(item,index) in form.Container" class="mtb20">
          <div>
            ...
            <el-form-item  >
              <el-button size="small" type="success" @click="addPort(index)" >添加端口</el-button>
            </el-form-item>
            <el-form-item  >
          ...
          <el-divider></el-divider>
          <h3>环境变量相关 <el-button size="small" type="success"  class="mf20" @click="addEnvFrom(index)" >添加环境引用变量</el-button></h3>
          ...
          <el-divider></el-divider>
          <h3>环境变量相关 <el-button size="small" type="success"  class="mf20" @click="addEnv(index)" >添加环境变量</el-button></h3>
          ...
          <el-divider></el-divider>
          <h3 >资源限制<el-button size="small" type="success"  class="mf20" @click="setResources(index)" >添加资源限制</el-button></h3>
          ...
          >
            <el-input v-model="item.resources.requests.memory" placeholder="memory限制" />
              ...
              <el-form-item  >
                <el-button size="small" type="danger" @click="removeResources(index)" >取消内存限制</el-button>
              </el-form-item>
          </div>
          </div>
          <el-divider></el-divider>
          ...
          <div v-if="item.livenessProbe&&item.livenessProbe.type"  class="mtb20">
            ...
            <div>
              ...
              <el-form-item  >
                <el-button size="small" type="danger" @click="removeResources(index)" >取消存活探针</el-button>
              </el-form-item>
            </div>
            <div>
              ...
              <div v-if="item.livenessProbe.type=='httpGet'">
                ...
                <el-form-item  >
                  <el-button size="small" type="success" @click="addHttpHeaders(index)" >添加header</el-button>
                </el-form-item>
...
export default defineComponent({
  ...
  setup: (props,{emit}) => {
    ...
    function removeResources(index){
    ...
    function addHttpHeaders(index) {
      state.form.Container[index].livenessProbe.httpGet.httpHeaders.push( {name:"",value:""})
    }
    ...
    function setData(list) {
      ...
      list.forEach((item,index)=>{
        ...
        result.livenessProbe=UnFilterLivenessProbe(item.livenessProbe,index)
        data.push(result)
      })
      state.form.Container=data
      commit()
    }
    function UnFilterLivenessProbe(item,index) {
        let result={
          initialDelaySeconds:1,
          timeoutSeconds:10,
          periodSeconds:1,
    ...
    function commit() {
      ...
      state.form.Container.forEach(item=>{
        ...
        result.livenessProbe=filterLivenessProbe(item.livenessProbe)
        data.push(result)
      })
    ...
    function filterLivenessProbe(item) {
      let result={}
      if(!item||!item.type){
        return result
      }
    ...
    function removeHeader(index,index2,name) {
      state.form.Container[index][name].httpGet.httpHeaders.splice(index2,1)
    }
    ...
    return {...toRefs(state),formRef,addContainers,requireRules,addPort,getObject,addEnvFrom,addEnv,setData,parseData,removeContainers,removeItem,setResources,setLivenessProbe,removeResources,addVolumeMounts,addHttpHeaders,removeHeader}
  }
})
</script>
